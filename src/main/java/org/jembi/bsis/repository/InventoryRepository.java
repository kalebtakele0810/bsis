...
public class InventoryRepository {
	...
	public List<StockLevelDTO> findStockLevelsForLocation(UUID locationId, InventoryStatus inventoryStatus) {
		return em
				.createNamedQuery(InventoryNamedQueryConstants.NAME_FIND_STOCK_LEVELS_FOR_LOCATION, StockLevelDTO.class)
				.setParameter("locationId", locationId).setParameter("deleted", false)
				.setParameter("inventoryStatus", inventoryStatus)
				.setParameter("statuses",
						Arrays.asList(ComponentStatus.AVAILABLE, ComponentStatus.QUARANTINED, ComponentStatus.EXPIRED))
				.getResultList();
	}
	...
	public List<StockLevelDTO> findStockLevels(InventoryStatus inventoryStatus) {
		return em.createNamedQuery(InventoryNamedQueryConstants.NAME_FIND_STOCK_LEVELS, StockLevelDTO.class)
				.setParameter("deleted", false).setParameter("inventoryStatus", inventoryStatus)
				.setParameter("statuses",
						Arrays.asList(ComponentStatus.AVAILABLE, ComponentStatus.QUARANTINED, ComponentStatus.EXPIRED))
				.getResultList();
	}
	...
	public Component findComponentByCodeAndDINInStock(String componentCode, String donationIdentificationNumber) {
		return em
				.createNamedQuery(ComponentNamedQueryConstants.NAME_FIND_COMPONENT_BY_CODE_AND_DIN_IN_STOCK,
						Component.class)
				.setParameter("donationIdentificationNumber", donationIdentificationNumber)
				.setParameter("componentCode", componentCode).getSingleResult();
	}
	...
	public List<Component> findComponentsInStock(UUID locationId, UUID componentTypeId, Date dueToExpireBy,
		...
		String queryString = "SELECT c.* FROM Component c WHERE c.isDeleted = :isDeleted AND c.inventoryStatus = :inventoryStatus  ";
		...
		query.setParameter("inventoryStatus", "IN_STOCK");
