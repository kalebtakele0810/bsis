...
public class ComponentCRUDService {
	...
	public Component createInitialComponent(Donation donation) {
		...
		// Create initial component only if the countAsDonation is true and the config
		// option is enabled
		...
		// set new component creation date to match donation date and bleedStartTime if
		// specified
		...
		// set new component expiresOn date to be 'expiresAfter' days after the
		// createdOn date
		component.setExpiresOn(calculateExpiresOn(component.getCreatedOn(), componentType.getExpiresAfter(),
				componentType.getExpiresAfterUnits()));

		// set the component venue and component batch (if already created for the
		// donation batch)
		ComponentBatch componentBatch = donationBatchRepository
				.findComponentBatchByDonationbatchId(donation.getDonationBatch().getId());
	...
	/**
	 * Change the specified Component's PackType and update. Note: this will change
	 * the Component's type, the Component's code and the expiresOn date
	 * 
	 * @param newPackType
	 *            PackType Donation's updated PackType
	 * @param component
	 *            Component to update
	...
	public Component updateComponentWithNewPackType(Component component, PackType newPackType) {
		...
		component.setExpiresOn(calculateExpiresOn(component.getCreatedOn(), newComponentType.getExpiresAfter(),
				newComponentType.getExpiresAfterUnits()));
	...
	/**
	 * Change the status of components belonging to the donor from AVAILABLE to
	 * UNSAFE.
	 */
	...
	/**
	 * Change the status of components linked to the donation from AVAILABLE to
	 * UNSAFE.
	 */
	...
	public Component processComponent(UUID parentComponentId, UUID componentTypeCombinationId, Date processedOn) {
		...
		ComponentTypeCombination componentTypeCombination = componentTypeCombinationRepository
				.findComponentTypeCombinationById(componentTypeCombinationId);
		...
		// condition added by kaleb
		if (parentComponent.getStatusChanges() != null) {
			for (ComponentStatusChange statusChange : parentComponent.getStatusChanges()) {
				...
				if (statusChange.getStatusChangeReason().getType() == ComponentStatusChangeReasonType.LOW_WEIGHT
						|| statusChange.getStatusChangeReason()
								.getType() == ComponentStatusChangeReasonType.INVALID_WEIGHT) {
					throw new IllegalStateException("Component " + parentComponentId + " cannot be processed.");
				}
			}
		}
		...
		// iterate over components in combination, adding them to the new components
		// map, along with the
	 ...
	 *
	 * If the parent component is unsafe then the child component will be marked as
	 * unsafe (using the reason UNSAFE_PARENT) except where the status change reason
	 * type is TEST_RESULTS_CONTAINS_PLASMA and this component doesn't contain
	 * plasma.
	 *
	 * If the donation bleed time exceeds the maximum bleed time, or if the time
	 * since the donation exceeds the max time since donation, for this component
	 * type then this component will be marked as unsafe using
	 * EXCEEDS_MAX_BLEED_TIME or EXCEEDS_MAXTIME_SINCE_DONATION
	 *
	 * @param component
	 *            the component
	 */
	private void markChildComponentAsUnsafeWhereApplicable(Component component) {
		...
		// If the component was processed more than once, get the initial component as
		...
		if (initialComponent.getStatusChanges() != null) {
			for (ComponentStatusChange statusChange : initialComponent.getStatusChanges()) {
				...
				if (!component.getComponentType().getContainsPlasma()
						&& statusChange.getStatusChangeReason()
								.getCategory() == ComponentStatusChangeReasonCategory.UNSAFE
						&& (statusChange.getStatusChangeReason()
								.getType() == ComponentStatusChangeReasonType.TEST_RESULTS_CONTAINS_PLASMA
								|| statusChange.getStatusChangeReason()
										.getType() == ComponentStatusChangeReasonType.LOW_WEIGHT)) {
					// skip because the component doesn't contain plasma and the unsafe reason only
					// applies to
		...
		if (component.getComponentType().getMaxBleedTime() != null
			...
			if (initialComponent.getProcessedOn() == null) {
				LOGGER.warn("ProcessedOn date is null for donation Id:" + donation.getId() + " with DIN: "
						+ donation.getDonationIdentificationNumber() + ", time since donation check is ignored");
			} else {
				long timeSinceDonation = bleedTimeService.getTimeSinceDonation(initialComponent.getCreatedOn(),
						initialComponent.getProcessedOn());
	...
	public Component undiscardComponent(UUID componentId) {
		...
		// As the component is being undiscarded, the status change with category
		// DISCARDED needs to be
	...
	public Component preProcessComponent(UUID componentId, Integer componentWeight, Date bleedStartTime,
			Date bleedEndTime) {
		Component existingComponent = componentRepository.findComponentById(componentId);
		...
		// note: existingComponent is the initial component because pre-processing is
		// only done for the initial component
		...
		if (!componentConstraintChecker.canPreProcess(existingComponent)) {
			throw new IllegalStateException("The component " + componentId + " cannot be pre-processed");
		}
		// it's OK to update the weight
		existingComponent.setWeight(componentWeight);

		// the weight has been updated, so we need roll back unsafe component status
		// changes with type
		// INVALID_WEIGHT and LOW_WEIGHT, as new component status changes will be
		// created where
		...
		if (componentStatusCalculator.shouldComponentBeDiscardedForInvalidWeight(existingComponent)) {
			existingComponent = markComponentAsUnsafe(existingComponent,
	...
	public Component recordChildComponentWeight(UUID componentId, Integer componentWeight) {
		...
		if (!componentConstraintChecker.canRecordChildComponentWeight(existingComponent)) {
			throw new IllegalStateException("The weight of child component " + componentId + " cannot be updated");
		}
		// it's OK to update the weight
		existingComponent.setWeight(componentWeight);
	...
	public Component unprocessComponent(Component parentComponent) {
		...
		// FIXME: Create component status change for when processing a component. Once
		// this is done, the
		// status change with category "PROCESSED" should be deleted when unprocessing a
		// component. In
		// the meantime we will call the method rollBackComponentStatusChanges with
		// category null
	...
	public Component markComponentAsUnsafe(Component component, ComponentStatusChangeReasonType reasonType) {
		...
		// Create a component status change, with category UNSAFE and type reasonType,
		// for the component
		ComponentStatusChange statusChange = new ComponentStatusChange();
		...
		try {
			unsafeReason = componentStatusChangeReasonRepository.findComponentStatusChangeReasonByCategoryAndType(
					ComponentStatusChangeReasonCategory.UNSAFE, reasonType);
		} catch (NoResultException e) {
			throw new IllegalArgumentException(
					"Component status change reason with category UNSAFE and type " + reasonType + " doesn't exist");
		}
	...
	public Component issueComponent(Component component, Location issueTo) {
		if (!issueTo.getIsUsageSite()) {
			throw new IllegalArgumentException(
					"Can't issue a component to a location which is not a usage site: " + issueTo);
		}
	...
	public Component transferComponent(Component component, Location transferTo) {
		if (!transferTo.getIsDistributionSite()) {
			throw new IllegalArgumentException(
					"Can't transfer a component to a location which is not a distribution site: " + transferTo);
		}
	...
	public Component transfuseComponent(Component component) {
		...
		if (!componentConstraintChecker.canTransfuse(component)) {
			throw new IllegalStateException(
					"Component " + component.getId() + " is in the wrong state to be transfused.");
		}
	...
	/**
	 * Roll back component status changes. This method is called whenever component
	 * status changes need to be deleted:
	 * 
	 * - when un-discarding: roll back status changes with category = DISCARDED (no
	 * type to specify)
	 * 
	 * - when updating the weight: roll back status changes with category = UNSAFE
	 * with types INVALID_WEIGHT and LOW_WEIGHT
	 * 
	 * - when un-processing: No status changes are created when processing at the
	 * moment, so there's nothing to roll back
	 * 
	 * Note: only status changes with types that can be rolled back will be deleted.
	 *
	 * @param component
	 *            Component for which status changes are being rolled back
	 * @param statusChangeReasonCategory,
	 *            the status change category that is being deleted (can be null if
	 *            no ComponentStatusChanges should be deleted)
	 * @param statusChangeReasonTypes,
	 *            the list of reason types that needs to be deleted for the
	 *            specified category (can be null if no ComponentStatusChanges
	 *            should be deleted)
		...
		if (component.getStatusChanges() != null) {
			for (ComponentStatusChange statusChange : component.getStatusChanges()) {
				if (!statusChange.getIsDeleted()) {
					...
					// If an UNSAFE status change that can't be rolled back is found, set the
					// component status to UNSAFE
					if (statusChange.getStatusChangeReason().getCategory()
							.equals(ComponentStatusChangeReasonCategory.UNSAFE)
							&& !ComponentStatusChangeReasonType
									.canBeRolledBack(statusChange.getStatusChangeReason().getType())) {
					...
					ComponentStatusChangeReasonType statusChangeReasonType = statusChange.getStatusChangeReason()
							.getType();
					if (statusChange.getStatusChangeReason().getCategory().equals(statusChangeReasonCategory)
						...
						// rollBackCategory matches and this statusChange reason type is able to be
						// rolled back
	...
	/**
	 * Change the status of components linked to the donation from AVAILABLE to
	 * UNSAFE only If they contains Plasma.
	 */
	...
	public Component untransfuseComponent(Component component) {
		...
		if (!component.getStatus().equals(ComponentStatus.TRANSFUSED)) {
			throw new IllegalStateException("Component " + component.getId() + " with status " + component.getStatus()
					+ " must have TRANSFUSED status");
		}
	...
	private void createChildComponent(ComponentType pt, int noOfUnits, Component parentComponent) {
		...
		for (int i = 1; i <= noOfUnits; i++) {
			...
			component.setExpiresOn(calculateExpiresOn(donationBleedDateTime.toDate(), pt.getExpiresAfter(),
					pt.getExpiresAfterUnits()));
